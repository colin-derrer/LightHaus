...
function getCheckedLanguages(
  ...
  return allLanguages
    .filter((language) => checkedLanguages.includes(language.value))
...
const FormSchema = z.object({
  languages: z
    .enum(["en", "fr", "de", "es", "pt", "ru", "ja", "ko", "zh"] as const)
    .array(),
}) satisfies z.ZodType<{ languages: (typeof languages)[number]["value"][] }>;

export default function TestingSearchForm() {
    ...
    toast({
      ...
      description: (
        <pre className="mt-2 w-[340px] rounded-md bg-slate-950 p-4">
          Toast.
          <code className="text-white">{data.languages}</code>
        </pre>
      ),
    });
  }
